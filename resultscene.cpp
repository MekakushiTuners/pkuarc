...
void resultscene::initscene(){
    ...
    connect(&pixlabel[15],&QLabel_C::clicked,this,&resultscene::backtomain);
    ...
    drawtext(&tex[17],QFont("Noto Sans CJK SC Bold", 13, false, false),131,1020,"主菜单");
    connect(&tex[17],&QLabel_C::clicked,this,&resultscene::backtomain);
}
...
void resultscene::openslot(int num,int sc,int mx,int pr,int fr,int lo){
    songnumber = num;
    score = sc;
    maxrecall = mx;
    pure = pr;
